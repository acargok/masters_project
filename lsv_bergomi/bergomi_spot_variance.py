...
def compute_chi(t, T, kappa1, kappa2, theta_param, rho12, alpha_theta):
    """chi(t,T) = Var[x^T_t] from the OU variances/covariance (Wang 3.8):
    Var[X^i_t]=(1-e^{-2 kappa_i t})/(2 kappa_i),
    Cov=(rho12)(1-e^{-(k1+k2)t})/(k1+k2), combined with the theta/alpha_theta
    weights and e^{-kappa(T-t)} decay factors."""
    th = theta_param
    tau = T - t
...
def compute_spot_variance(X1, X2, t, bergomi, fwd_var_interp, ttm_grid):
    """Spot variance xi^t_t per particle:
        ...
        x^t_t = alpha_theta[(1-theta)X1 + theta X2]  (T=t, decay factors =1).

    X1, X2: OU states (N,). fwd_var_interp: xi^T_0 interpolator. ttm_grid for
    clamping. Returns spot variance (N,)."""
    nu = bergomi["nu"]
    theta = bergomi["theta"]
    kappa1 = bergomi["kappa1"]
    kappa2 = bergomi["kappa2"]
    rho12 = bergomi["rho12"]
    # omega = 2*nu: Wang's nu (vol-of-vol formula 3.10) is half the f_T
    # coefficient, per the Bergomi 2008 single-factor convention.
    omega = 2.0 * nu
