...
# -*- coding: utf-8 -*-
"""
Bergomi LSV Validation — Checkpoint 2
=======================================
Master's Thesis, Imperial College London.

Validates the calibrated leverage function sigma(t, S) under Bergomi LSV
by repricing vanilla European options via Monte Carlo and comparing to
market IVs and Dupire repricing results from Step 2.

Bergomi LSV dynamics:
    dS/S = (r-q) dt + sigma(S,t) sqrt(xi^t_t) dW^S

where xi^t_t is the Bergomi spot variance. The leverage function sigma(S,t)
is interpolated from the surface computed in the particle method step.

Inputs:
    lsv_bergomi/arrays/leverage_*.npy    — calibrated leverage
    lsv_bergomi/data/bergomi_params.json — model parameters
    lsv_bergomi/arrays/fwd_var_curve.npy — forward variance
    dupire_vol/data/repricing_errors.csv — Dupire baseline
    iv_surface/data/spx_iv_data.csv      — option data

Outputs:
    lsv_bergomi/data/lsv_repricing_errors.csv
    lsv_bergomi/data/validation_summary.json
    lsv_bergomi/plots/lsv_repricing_validation.png
"""
...
def bergomi_lsv_mc_reprice(inputs, n_paths=MC_N_PATHS,
    ...
    # Diagnostic: log det + eigenvalues of input matrix.
    ...
    # Validation always regularises via eigenvalue floor at 1e-6
    ...
    # Diagnostic: post-regularisation correlation + det.
    ...
    for _, row in sample.iterrows():
        ...
        # `iv_ssvi` in repricing_errors.csv is the SSVI-fitted IV (sourced
        # from spx_iv_data.csv:iv, which is the SSVI parameterisation).
        # We rebuild the SSVI BS price from it — this is the comparison
        # target instead of the raw market mid.
        iv_ssvi = row["iv_ssvi"]
        if opt_type == "call":
